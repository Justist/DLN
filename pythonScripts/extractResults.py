...
for fullfilename in glob(f"{targetdir}*.sorted"):
    with open(fullfilename, "r") as file:
        ...
        for line in lines:
            ...
            if value < lowestvalue:
                lowestvalue = value;
                lowestscheme = scheme
    with open(f"{targetdir}e{epoch}.extracted", "w") as n:
        n.write(
            (
                (
                    (
                        (
                            f"first,{first}last,{last}highest,{highestscheme},{str(highestvalue)}"
                            + "\nlowest,"
                        )
                        + lowestscheme
                    )
